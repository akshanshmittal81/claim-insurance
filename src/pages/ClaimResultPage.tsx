...
import { CheckCircle2, XCircle, Clock, MapPin, Phone, Star, Banknote, ExternalLink, RefreshCw, ChevronRight, Lightbulb } from 'lucide-react'
...
export default function ClaimResultPage() {
  ...
  if (!result) return (
    <AppLayout>
      <div className="max-w-2xl mx-auto rounded-3xl p-16 text-center" style={{ background: 'rgba(255,255,255,0.9)', border: '1px solid #DBEAFE' }}>
  ...
  const fraudBarColor = result.fraudRiskLabel === 'low' ? 'linear-gradient(90deg,#059669,#10B981)' : result.fraudRiskLabel === 'medium' ? 'linear-gradient(90deg,#D97706,#F59E0B)' : 'linear-gradient(90deg,#DC2626,#EF4444)'
  const DecisionIcon = result.decision === 'approved' ? CheckCircle2 : result.decision === 'rejected' ? XCircle : Clock
  ...
  return (
    <AppLayout>
      <Breadcrumb items={[{ label: 'Dashboard', href: '/dashboard' }, { label: claimIdToDisplay(result.claimId) }, { label: 'Result' }]} />
      <div className="max-w-2xl mx-auto space-y-4">

        {/* Decision Banner */}
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}
          className="rounded-3xl p-6" style={{ background: decisionGradient.bg, border: `1.5px solid ${decisionGradient.border}`, boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="rounded-3xl p-5" style={{ background: 'rgba(255,255,255,0.9)', border: '1px solid #DBEAFE', boxShadow: '0 4px 20px rgba(59,130,246,0.06)' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <div className="h-2.5 rounded-full overflow-hidden mb-3" style={{ background: '#F1F5F9' }}>
              <motion.div className="h-full rounded-full"
                style={{ background: fraudBarColor }}
        ...
        {result.explainability.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="rounded-3xl p-5" style={{ background: 'linear-gradient(135deg, #FFFBEB, #FEF3C7)', border: '1px solid #FDE68A', boxShadow: '0 4px 20px rgba(245,158,11,0.08)' }}>
        ...
        {result.garage && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            className="rounded-3xl p-5" style={{ background: 'linear-gradient(135deg, #F0FDF4, #DCFCE7)', border: '1px solid #BBF7D0', boxShadow: '0 4px 20px rgba(16,185,129,0.08)' }}>
            <div className="flex items-center gap-2 mb-4">
        ...
        {result.payment && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            {result.payment.transactionId && <p className="text-xs font-mono text-slate-400 mt-1">Txn: {result.payment.transactionId}</p>}
          </motion.div>
        )}
